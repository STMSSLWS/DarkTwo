...
const { thresholdSettings } = require('../conf/setting')
...
module.exports = {
    ...
    CreateActivity: async function (req, res) {
        let { activityName, location, startTime, remarks, deviceSelections } = req.body
    ...
    GetPastActivityBySelectActivityId: async function (req, res) {
        let { selectActivityId, start, length } = req.body
    ...
    UploadMobileData: async function (req, res) {
        try {
            let { ipAddress, semId, semStatus, sTemp, cTemp, psi, hr, br, lat, lng, datetime, thresholdAlert, deviceStatus, activityId, hrConfidence, coreTempConfidence } = req.body
    ...
    GetThresholdSettings: async function (req, res) {
        try {
            return res.json(utils.responseSuccess(thresholdSettings))
        } catch (ex) {
            log.error(ex)
