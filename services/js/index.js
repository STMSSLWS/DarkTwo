...
var chart;
var tcRedThresholdSlider,
    tcAmberThresholdSlider,
    psiRedThresholdSlider,
    psiAmberThresholdSlider;
...
const GetCurrentActivity = function () {
    axios.post('/getCurrentActivity').then(async result => {
        ...
        if (data.activityStatus == "Start") {
            ...
            var timerTrainees = setInterval(function () {
                refreshTrainees()
            }, frequency)
...
window.panToMap = function (e, lat, lng, ipAddress) {
    if (lat && lng) {
...
const temp = `<div class="col trainee-container" data-status="{{bg}}" data-id="{{id}}" data-tc="{{tc1}}" {{style}} onclick="panToMap(this, {{lat}}, {{lng}}, '{{ipAddress}}')">
...
function sortList() {
    ...
    list = document.getElementById("trainee-detail");
    switching = true;
    while (switching) {
        switching = false;
        b = list.getElementsByClassName("trainee-container");
        for (i = 0; i < (b.length - 1); i++) {
            shouldSwitch = false;
            // if ($(b[i]).attr("data-status") < $(b[i + 1]).attr("data-status")
            //     || $(b[i]).attr("data-status") == $(b[i + 1]).attr("data-status") && Number($(b[i]).attr("data-id")) > Number($(b[i + 1]).attr("data-id"))) {
            //     shouldSwitch = true;
            //     break;
            // }
            if ($(b[i]).attr("data-status") == $(b[i + 1]).attr("data-status") && Number($(b[i]).attr("data-tc")) < Number($(b[i + 1]).attr("data-tc"))) {
                shouldSwitch = true;
                break;
            }
        }
...
const InitTcSlider = function (tcRedThresholdVal, tcAmberThresholdVal) {
    ...
    layui.use('slider', function () {
        ...
        tcRedThresholdSlider = slider.render({
            min: 370,
            max: 420,
            tips: false,
            elem: '#tcRedThreshold',
        ...
        tcAmberThresholdSlider = slider.render({
            min: 370,
            max: 420,
            tips: false,
...
const InitPSISlider = function (psiRedThresholdVal, psiAmberThresholdVal, disabled) {
    ...
    layui.use('slider', function () {
        var $ = layui.$, slider = layui.slider;
        psiRedThresholdSlider = slider.render({
            min: 0,
            max: 100,
            tips: false,
        ...
        psiAmberThresholdSlider = slider.render({
            min: 0,
            max: 100,
            tips: false,
